<div align="center">

<img src="./public/reactpilot-ui-logo.png" width="140" alt="ReactPilot-UI Logo" />

# ⚡ ReactPilot-UI

### Build React UIs at Lightning Speed

Modern React UI framework with beautiful components, templates, dashboards, animations, and developer tools.

<br/>

<p align="center">
  <img src="https://img.shields.io/npm/v/reactpilot-ui?color=blue" />
  <img src="https://img.shields.io/github/stars/rajeshnraj2004/reactpilot-ui?style=social" />
  <img src="https://img.shields.io/github/license/rajeshnraj2004/reactpilot-ui" />
  <img src="https://img.shields.io/badge/react-19-blue" />
  <img src="https://img.shields.io/badge/vite-fast-purple" />
  <img src="https://img.shields.io/badge/tailwindcss-modern-cyan" />
</p>

<br/>

<img src="./preview.png" alt="ReactPilot-UI Preview"/>

</div>

---

# ✨ Features

- ⚡ Lightning Fast
- 🎨 Modern UI Components
- 🌙 Dark Mode Support
- 📱 Fully Responsive
- 🧩 Plug & Play Components
- 🚀 Production Ready
- 🎬 Smooth Animations
- 🔥 Tailwind CSS Powered
- 🛠 Easy Customization
- 📦 One-Line Installation
- 📊 Dashboard Templates
- 🤖 AI UI Components
- 🎯 Developer Friendly API

---

# 📦 Installation

## npm

```bash
npm install reactpilot-ui
```

## yarn

```bash
yarn add reactpilot-ui
```

## pnpm

```bash
pnpm add reactpilot-ui
```

---

# 🚀 Quick Start

```jsx
import { Dashboard } from "reactpilot-ui"

export default function App() {
  return <Dashboard />
}
```

---

# ⚙️ Setup With Vite

## Install Tailwind CSS

```bash
npm install tailwindcss @tailwindcss/vite
```

## vite.config.js

```js
import { defineConfig } from "vite"
import react from "@vitejs/plugin-react"
import tailwindcss from "@tailwindcss/vite"

export default defineConfig({
  plugins: [
    react(),
    tailwindcss(),
  ],
})
```

---

# 🎨 Components Included

## UI Components

- Buttons
- Cards
- Inputs
- Tables
- Forms
- Dropdowns
- Alerts
- Modals
- Sidebars
- Navbars
- Tabs
- Badges
- Tooltips

---

## Templates

- Admin Dashboard
- SaaS Dashboard
- Landing Pages
- Login UI
- Signup UI
- AI Chat UI
- Portfolio UI
- Analytics UI

---

# 💻 Example Usage

## Button Component

```jsx
import { Button } from "reactpilot-ui"

export default function App() {
  return (
    <Button variant="primary">
      Get Started
    </Button>
  )
}
```

---

## Dashboard Component

```jsx
import { Dashboard } from "reactpilot-ui"

export default function App() {
  return <Dashboard />
}
```

---

## Navbar Component

```jsx
import { Navbar } from "reactpilot-ui"

export default function App() {
  return (
    <Navbar
      title="ReactPilot-UI"
      links={[
        "Docs",
        "Components",
        "Templates",
      ]}
    />
  )
}
```

---

# 📂 Project Structure

```bash
reactpilot-ui/
│
├── components/
├── templates/
├── layouts/
├── hooks/
├── styles/
├── utils/
├── animations/
├── public/
└── docs/
```

---

# 🌈 Themes

ReactPilot-UI supports:

- Dark Theme
- Light Theme
- Neon Theme
- Glassmorphism
- Minimal UI
- Gradient Themes

---

# 🛠 Built With

- React
- Vite
- Tailwind CSS
- Framer Motion
- shadcn/ui
- Lucide Icons
- TypeScript

---

# 📖 Documentation

Coming Soon...

```bash
npm create reactpilot-ui
```

---

# ⚡ CLI Commands

Generate UI instantly.

```bash
npx reactpilot-ui create dashboard
```

```bash
npx reactpilot-ui create login
```

```bash
npx reactpilot-ui create landing-page
```

---

# 📸 Screenshots

## Dashboard UI

<img src="./screenshots/dashboard.png" alt="Dashboard UI"/>

---

## Landing Page

<img src="./screenshots/landing.png" alt="Landing Page"/>

---

## Login UI

<img src="./screenshots/login.png" alt="Login UI"/>

---

# 🚀 Roadmap

- [x] Core Components
- [x] Tailwind Support
- [x] Dashboard Templates
- [ ] CLI Generator
- [ ] AI UI Generator
- [ ] Theme Builder
- [ ] VS Code Extension
- [ ] React Native Support
- [ ] Figma Plugin

---

# 🤝 Contributing

## Clone Repository

```bash
git clone https://github.com/rajeshnraj2004/reactpilot-ui.git
```

## Go To Folder

```bash
cd reactpilot-ui
```

## Install Dependencies

```bash
npm install
```

## Start Development Server

```bash
npm run dev
```

---

# ⭐ Support

If you like this project:

- ⭐ Star this repository
- 🍴 Fork the project
- 🐛 Report bugs
- 💡 Suggest new features

---

# 📜 License

MIT License © 2026 ReactPilot-UI

---

<div align="center">

# 🚀 Build Faster. Design Better. Ship Smarter.

Made with ❤️ by Rajesh N

</div>
